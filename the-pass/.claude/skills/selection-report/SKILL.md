...
```bash
npx tsx scripts/sr-prep.ts --scout    # ★ 預設：RSS ＋ 查詢式進料（scout）← 每期這樣跑
npx tsx scripts/sr-prep.ts            # 省 credits／離線時才用：純 RSS（不含 scout）
```
...
**`--scout` ＝ 查詢式進料（Phase 1，2026-06-23 起設為每期預設）**：用 firecrawl 跑常駐 query（`src/lib/scout-queries.ts`，在地語言＋跨區主題）撈「RSS 看不到」的在地/驚奇題，產出與 RSS 同形狀的候選、併進同一個 pool（標 `origin:"scout"`、走同一套去重與評分）。配方/邊界見該檔註解。**省 firecrawl credits／離線時**才拿掉 `--scout`（回純 RSS），或把部分 query `enabled:false`。pool 裡 scout 項一樣交給步驟 2/3 篩——只「找題」不抓全文，全文留到 /write-issue。
