...
**選配·文章形式**：Terrel 若指定了文章形式／類型（人物專訪、現場特寫、評論、產業綜述、越洋觀點…），記下來——它在 step 4 注入給編輯當結構指引、step 5 讓總編多核一條「形式到位沒」。**不指定就略過**：多數委稿靠「選對編輯」自動帶形式（見 editor-team 的「文章類型 × 編輯配對」），不必明指；只有要某編輯寫他**非本命的形式**時（如讓 Lou 寫人物專訪而非評論）才值得指定。
...
> **選配·形式檢查（只在 step 0 指定了文章形式時觸發）**：總編在事實核查之外，多核**一條**「形式到位沒」——人物專訪→主角有沒有真的發聲（不是只來講一句就消失的引述機器）；產業綜述→有沒有一條數據/論點脊椎；現場特寫→有沒有到場的畫面感；評論（如 Lou）→判斷有沒有留白。**依指定形式對應一條，不是一整套 rubric。**
> **為什麼併在總編這關、不另開 subagent**：總編本來就是**獨立於寫稿編輯**的把關者（編輯寫、總編查），形式對不對正是它的職責，手上又已有草稿＋素材，零額外成本。需要「獨立隔離、避免自己替自己打分」的驗證才另開分身（如 voice-extract 的聲音 scorer——那是因為寫稿的分身會偏心）；形式核查沒有偏心問題，不必多開。
