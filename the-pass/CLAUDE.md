...
**十位 AI 編輯**：**四位原創**（虛構人設）＋ **六位真人原型**（取法真實名廚資深編輯）＋ 幕後總編。**總覽頁＝ [`editor-team.html`](public/editor-team.html)**（定位／特色／名廚類型配對／怎麼用／profile 概念）；寫作源頭頁＝ `editor-source.html`（gen，勿手改）。
...
**真人原型六位（取法·封閉——只讀自己人格檔、不借招庫，越像原作者越好）**

| 真人原型編輯 | 原型作者 | 筆調（需餵的素材） |
|---|---|---|
...
署名：長文「編輯／X」、快訊「彙整·Passe」、提問「問／Fumet」、特企「招待／Amuse」、真人原型編輯「編輯／<小名>」。
...
- **soul**（我是誰，`docs/editors/<名>-soul.md`）· **voices／style**（怎麼寫，`refs/voices.md`）· **anchors**（逐字筆調範本：真人原型編輯＝真人原文、原創＝自己最好稿〔選用，待補〕）· **moves**（命名招，真人原型編輯專有檔）· **memory**（學到/寫過，每期回寫）
- **核心模型 — 原創開放／真人原型編輯封閉**：`refs/craft-anchors.md` ＝ 從真人蒸餾的可轉移「招」**共用庫**，**只給原創四位讀**（博採眾招）；**真人原型編輯封閉**——只讀自己 4 檔（soul/anchors/moves/memory）＋ `anti-slop.md`，**不讀招庫**（借別人的招會四不像）。每跑一位新真人原型編輯（`/voice-extract`）就把他的招捐進招庫養原創。
...
- **Image:** 長文配圖 nanobanana（`mcp__nanobanana__generate_image`）＋ gpt-image（本機 **Codex CLI**）；快訊不配圖。**插圖風格＝可替換 profile**（`docs/illustration/styles/<name>/`，現役 `risograph`）；跨專案 **`/style-extract`**（user-level skill）抽風格＋生圖驗證、存共用庫 `~/style-lab/`。**文字版＝ `/voice-extract`**（專案 skill，`.claude/skills/voice-extract/`）：抽一位作者的筆調 → 做成 AI 真人原型編輯（見「AI 編輯團隊」）
...
├── .claude/skills/                ← selection-report · write-issue（含 refs/voices·anti-slop·chief-checklist）· publish-issue · audit-sources · **commission**（指定單一編輯單篇委稿，共用 write-issue 編輯檔）· **voice-extract**（抽作者筆調→做 AI 真人原型編輯，產出存 docs/editors/）
...
- **真人原型 ＋ `/voice-extract`（🆕 2026-06-24）**：把真實名廚編輯取法做成 AI真人原型編輯的 skill（style-extract 的文字版）。已建 **6 位**：Musubi←Mokki／Jang←Atomy／Bao←倫敦男子日常／Kaya←陳彬雁／Nano←章凱閎／Lou←謝嫣薇。流程＝讀語料→抽 voice-DNA(6 面向)→🚦gate(確認＋取小名)→真人原型編輯寫 fresh 稿→**獨立 scorer 對筆調錨打分**→🚦定稿→產真人原型編輯 4 檔＋捐招進共用 `craft-anchors.md`（現 14 招/7 作者）。**封閉/開放模型**（真人原型編輯只讀自己、原創讀招庫）。**取法個人、不取法機構**（MINGCHU 官方稿無單一人格→不做真人原型編輯、可當反面語料）。頁：`editor-team.html`（團隊總覽）、`voice-extract.html`（運作）。**待辦**：craft-anchors 已改 **by-category 分類**（5 類：開場／切角立意／結構推進／感官描寫／收尾＋〈按環節找招〉導引，14 招/7 作者，2026-06-24）；(選用)原創四位補 anchors（待累積代表作）。〔下批真人原型候選「謝嫣薇」已完成＝第六位真人原型編輯 **Lou（老火）**；定位＝考據鑑賞·判斷留白·不開槍。下一位候選待定。〕
...
- **真人原型編輯封閉、不讀 craft-anchors**: 真人原型編輯（Musubi/Jang/Bao/Kaya/Nano/Lou，`docs/editors/<名>-{soul,anchors,moves,memory}.md`）只注入自己 4 檔＋anti-slop，**不注入 voices 段、不注入 craft-anchors**（commission 依編輯類型分流）；只有原創四位讀招庫。混進去＝四不像。每位真人原型編輯守則都擋 mingchu 全站通病「格言式上升收尾」。
- **取法個人、不取法機構**: `/voice-extract` 只對「有一致可辨識筆調的個人作者」做真人原型編輯；機構署名容器（MINGCHU 官方稿）無單一人格→不做真人原型編輯（做了只是比 Passe 更無臉的通稿機），當反面語料即可。scout（讀 4–6 篇判題材/筆調/差異化）值得先跑、再決定做不做真人原型編輯。
