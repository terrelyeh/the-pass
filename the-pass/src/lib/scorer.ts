...
**編輯方向（團隊 2026-06-11 定）：飲食／餐飲／食物優先，AI／科技為輔與加分。** 飲食／餐飲／食物（廣義，含上下游與橫切面）是主角；AI／科技是讓題目更前沿、更獨特的加分，不是入選門檻。

第一步 · 硬閘門（pass）：需同時滿足
1. 主題落在「飲食／餐飲／食物**及相關**」領域（食物優先，**廣義**）——含上游（食材、農業）、中游（加工、製造、物流）、下游（餐廳、外送、零售）、橫切面（法規、設備、人力、行銷、永續），以及**「人」的故事——主廚、餐飲經營者、從業者、生產者**（正是「先找到人」的核心料）。不是只有「食物本身」。AI／科技不是必要條件、是加分；只擋掉「跟飲食完全無關」的（例：純無人機政策、跟飲食無連結的純 AI 研究；餐廳名含 "tail" 不算數）。
2. 事實可查、不是空泛炒作或純廣告。
